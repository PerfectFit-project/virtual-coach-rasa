...
def new_mocker_filter(mocker: MockerFixture) -> MagicMock:
    return mocker.MagicMock(return_value=mocker.MagicMock(**{"limit": mocker.MagicMock(**{"all": mocker.MagicMock()})}))
...
@pytest.fixture
def mocker_get_user_intervention_activity_inputs(mocker: MockerFixture) -> MagicMock:
    return mocker.patch("Rasa_Bot.actions.actions_general_activity.get_user_intervention_activity_inputs")
...
def test_get_user_intervention_activity_inputs(
    ...
    inputs = get_user_intervention_activity_inputs(user_id, last_activity_id_slot)
...
async def test_run_action_general_activity_check_rating__has_record_and_low(
    ...
    mocker_db_session.query.return_value.filter.return_value.all = mocker.MagicMock(return_value=current_record)
    ...
    mocker_db_session.query.return_value.order_by.call_args.args[0].compare(FirstAidKit.activity_rating.desc())
...
async def test_run_action_general_activity_check_rating__no_record(
    ...
    assert mocker_db_session.add.call_args.args[0].users_nicedayuid == tracker.current_state()["sender_id"]
    assert mocker_db_session.add.call_args.args[0].intervention_activity_id == last_activity_id_slot
    assert mocker_db_session.add.call_args.args[0].activity_rating == int(activity_useful_rating)
...
def test_get_random_activities(mocker: MockerFixture, mocker_db_session: MagicMock) -> None:
    avoid_activity_id = 1
    ...
    mocker_db_session.query.return_value.filter.return_value.all.return_value = [
        ...
        InterventionActivity(intervention_activity_id=6),
    ]
    random_activities = get_random_activities(avoid_activity_id, number_of_activities)
...
async def test_run_action_get_three_random_activities(
    ...
    mocker_get_random_activities = mocker.patch("Rasa_Bot.actions.actions_general_activity.get_random_activities")
    mocker_get_random_activities.return_value = random_three_activities

    events = await action.run(dispatcher, tracker, domain)

    mocker_get_random_activities.assert_called_once_with(last_activity_id_slot, 3)
    assert events == [
        SlotSet("activity1_name", random_three_activities[0].intervention_activity_title),
        SlotSet("activity2_name", random_three_activities[1].intervention_activity_title),
        SlotSet("activity3_name", random_three_activities[2].intervention_activity_title),
        SlotSet("rnd_activities_ids", [activity.intervention_activity_id for activity in random_three_activities]),
    ]
...
async def test_run_action_get_last_performed_activity__no_last(
    ...
    last_activity = InterventionActivity(
        intervention_activity_title=last_activity_title, intervention_activity_id=intervention_activity_id
    )
...
async def test_run_action_get_last_performed_activity__has_last(
    ...
    assert events == [SlotSet("last_activity_slot", None), SlotSet("last_activity_id_slot", None)]
...
async def test_run_action_load_activity__no_inputs(
    ...
    tracker.add_slots([SlotSet("general_activity_next_activity_slot", "2"), SlotSet("rnd_activities_ids", [1, 5, 3])])
    ...
    mocker_get_user_intervention_activity_inputs.assert_called_once_with(tracker.current_state()["sender_id"], 5)
    mocker_db_session.add.assert_called_once()

    assert mocker_db_session.add.call_args.args[0].users_nicedayuid == tracker.current_state()["sender_id"]
    assert mocker_db_session.add.call_args.args[0].intervention_activity_id == 5
    assert mocker_db_session.add.call_args.args[0].user_input == None
